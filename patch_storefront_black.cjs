...
const targetHeaderStart = `<header className="bg-white text-black sticky top-0 z-50 border-b border-slate-200 shadow-sm">`;
...
if (startIndex !== -1 && endIndex !== -1) {
  const newHeader = `<header className="bg-black text-white sticky top-0 z-50">
          ...
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-6">
              ...
              <div className="font-black text-2xl tracking-tighter cursor-pointer flex items-center" onClick={() => setActiveTab?.('Orders')}>
                <span className="text-red-600">S</span><span className="text-white">R</span><span className="text-red-600">K</span>
            ...
            <div className="hidden md:flex items-center space-x-8 flex-1 justify-center">
              {categories.slice(0, 5).map(cat => (
                <button 
                  ...
                  className={\`text-sm font-bold uppercase tracking-wider transition-colors \${selectedCategory === cat ? 'text-red-600' : 'text-slate-400 hover:text-white'}\`}
                >
                  {cat as string}
                </button>
              ))}
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="relative hidden sm:block">
                ...
                <input 
                  ...
                  className="bg-slate-900 border border-slate-800 text-white rounded-full pl-9 pr-4 py-1.5 text-sm focus:outline-none focus:border-slate-700 transition-colors w-48 placeholder:text-slate-500"
                />
              </div>
            </div>
          </div>
  ...
  console.log("Patched StorefrontTab header to black");
} else {
